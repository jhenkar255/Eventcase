...
const categoryIcons: Record<string, React.ComponentType<{ size?: number | string; className?: string }>> = {
  ...
  'Cultural Event': Palette, Other: Sparkles,
};

const howItWorks = [
  { step: '1', title: 'Create your event', desc: 'Set the date, guest count and budget. Add schedules and invite guests with RSVP tracking.' },
  { step: '2', title: 'Discover & book', desc: 'Browse verified venues and vendors, compare prices and reviews, and book in a few clicks.' },
  { step: '3', title: 'Pay securely', desc: 'Confirm bookings with secure payments and track every rupee in your budget planner.' },
  { step: '4', title: 'Celebrate & review', desc: 'Enjoy your flawless event, then share your experience to help the community.' },
];
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-primary-700 via-primary-600 to-indigo-700">
        <div className="pointer-events-none absolute -top-32 right-0 h-96 w-96 rounded-full bg-white/10 blur-3xl" aria-hidden />
        <div className="pointer-events-none absolute bottom-0 left-0 h-80 w-80 rounded-full bg-indigo-400/20 blur-3xl" aria-hidden />
        <div className="relative mx-auto max-w-7xl px-4 py-20 text-center sm:px-6 sm:py-28 lg:px-8">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/25 bg-white/10 px-4 py-1.5 text-xs font-bold text-white backdrop-blur">
          ...
          <form onSubmit={doSearch} className="mx-auto mt-12 max-w-4xl rounded-2xl bg-white p-4 shadow-xl sm:p-5" aria-label="Search venues">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              ...
              />
              <Input id="hero-location" label="Location" placeholder="City" value={search.location} onChange={(e) => setSearch((s) => ({ ...s, location: e.target.value }))} />
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
          {EVENT_TYPES.filter((t) => t !== 'Other').map((type) => {
            ...
            return (
              ...
              >
                <span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary-50 text-primary-600 transition group-hover:bg-primary-600 group-hover:text-white">
                  <Icon size={22} />
                </span>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <SectionTitle
          ...
          subtitle="Trusted professionals for every need"
          action={
      ...
      <section className="bg-slate-900 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 text-center">
            ...
            <p className="mt-2 text-slate-400">From idea to celebration in four simple steps</p>
          </div>
          <div className="grid gap-6 md:grid-cols-4">
            {howItWorks.map((s, i) => (
              <div key={s.step} className="relative rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
                <span className="absolute -top-4 left-6 flex h-9 w-9 items-center justify-center rounded-full bg-primary-500 text-sm font-extrabold shadow-lg">
                  {s.step}
                </span>
                {i < 3 && <ArrowRight size={18} className="absolute -right-4 top-1/2 hidden -translate-y-1/2 text-slate-600 md:block" aria-hidden />}
      ...
      <section className="border-y border-slate-100 bg-white py-14">
        <div className="mx-auto grid max-w-5xl grid-cols-2 gap-8 px-4 text-center md:grid-cols-4 sm:px-6">
          {[
            { icon: Store, label: 'Verified vendors', value: stats?.vendors ?? 0 },
            { icon: Building2, label: 'Premium venues', value: stats?.venues ?? 0 },
            { icon: CalendarDays, label: 'Events planned', value: stats?.events ?? 0 },
            { icon: CheckCircle2, label: 'Bookings completed', value: stats?.bookings ?? 0 },
          ].map(({ icon: Icon, label, value }) => (
            <div key={label}>
              <Icon size={26} className="mx-auto text-primary-500" aria-hidden />
      ...
      {/* ---------- CTA ---------- */}
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary-600 to-indigo-600 px-8 py-14 text-center text-white sm:px-16">
          <Quote size={120} className="pointer-events-none absolute -left-6 -top-6 opacity-10" aria-hidden />
          <h2 className="text-3xl font-extrabold sm:text-4xl">Ready to plan something unforgettable?</h2>
          <p className="mx-auto mt-3 max-w-xl text-primary-100">
            Join thousands of organizers who trust EventEase for weddings, conferences, birthdays and everything in between.
          </p>
...
const VendorCardMini = ({ vendor }: { vendor: Vendor }) => {
  ...
  return (
    <Link to={`/vendors/${vendor._id}`} className="card-base group block p-5 transition hover:-translate-y-0.5 hover:shadow-card-hover">
      <div className="flex items-start justify-between">
        <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-50 text-primary-600 transition group-hover:bg-primary-600 group-hover:text-white">
          <Icon size={21} />
        </span>
        {vendor.rating > 0 && (
          <span className="flex items-center gap-1 text-xs font-bold text-amber-600">
            <Star size={13} fill="#f59e0b" className="text-amber-500" /> {vendor.rating.toFixed(1)}
          </span>
        )}
      </div>
      <h3 className="mt-3 truncate font-bold text-slate-900">{vendor.businessName}</h3>
      <p className="mt-0.5 text-xs font-semibold uppercase tracking-wide text-slate-400">{vendor.category}</p>
